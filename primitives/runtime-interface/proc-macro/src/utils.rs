...
use syn::{
	parse::Parse, parse_quote, spanned::Spanned, token, Attribute, Error, FnArg, Ident, ItemTrait,
	Lit, LitInt, Meta, NestedMeta, Pat, PatType, Result, Signature, TraitItem, TraitItemMethod,
	Type,
};
...
impl<'a> RuntimeInterfaceFunction<'a> {
	fn new(version: VersionAttribute, trait_item: &'a TraitItemMethod) -> Self {
		Self {
			latest_version_to_call: version.register_only.map(|_| version.version),
...
impl<'a> RuntimeInterface<'a> {
	/// Retunrs an iterator over all runtime interface function
	...
	for item in get_trait_methods(trait_def) {
		...
		match functions.entry(name.clone()) {
			...
			Entry::Occupied(mut entry) => {
				...
				if interface_item.latest_version_to_call < version {
					interface_item.latest_version_to_call = version;
				}
